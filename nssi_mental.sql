...
select ?nssi ?mental ?what (COUNT(distinct ?pubmed) as ?count) 
where {
    ?t1s1 ztonekg:SenseURL snomed:248062006.  #snomedCT code for NSSI
    ?t1s1 ztonekg:SenseURL ?nssi.
    ...
    ?t1s1b ztonekg:SenseURL snomed:74732009. #snomedCT code for mental illnesses
    ?t1s1b ztonekg:SenseURL ?mental.
    ?t1s1c ztonekg:SenseURL ?what.
    ?s1b ztonekg:hasText ?title.   
}
GROUP BY ?mental ?nssi ?what
ORDER by DESC(?count)
